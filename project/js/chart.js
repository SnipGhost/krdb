...
function pieChart() {
  ...
  function drawSlice ( context, slice ) {
    ...
    if ( slice == currentPullOutSlice ) {
      ...
      var str = "" + chartData[slice]['value'];
      str.substring(0, str.length - 3);								//МОИ ПОПРАВКИ
      var val_s = str*1;
      context.fillText( /*pullOutValuePrefix + chartData[slice]['value']*/ val_s + " (" + ( parseInt( chartData[slice]['value'] / totalValue * 100 + .5 ) ) +  "%)", centreX + Math.cos(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ), centreY + Math.sin(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ) + 20 );
